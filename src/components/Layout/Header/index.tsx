...
const Header: React.FC = () => {
  ...
  }, [handleScroll, handleEscapeKey, pathname]);

  const isHomepage = pathname === "/";

  // Добавляем класс для body чтобы предотвратить скролл при открытом меню
  useEffect(() => {
    if (navbarOpen) {
  ...
  return (
    <header
      className={`fixed h-16 sm:h-20 md:h-24 py-1 z-50 w-full bg-transparent transition-all duration-300 lg:px-0 px-3 sm:px-4 ${
        sticky ? "top-2 sm:top-3" : "top-0"
      }`}
    >
      <nav
        className={`container mx-auto max-w-8xl flex items-center justify-between py-3 sm:py-4 duration-300 ${
          sticky
            ? "shadow-lg bg-white rounded-full top-5 px-4 sm:px-6 md:px-8"
            : "shadow-none top-0"
        }`}
      >
        <div className="flex justify-between items-center gap-2 w-full">
          ...
          <div className="flex items-center gap-2 sm:gap-6">
            <div className={`hidden md:block text-xs md:text-sm lg:text-base`}>
              <Link
                href="tel:+79950613997"
                className="flex items-center gap-2 border-r pr-4 md:pr-6 text-dark hover:text-primary border-dark/20"
              >
                <Icon
                  icon={"ph:phone-bold"}
                  width={20}
      ...
      <div
        ref={sideMenuRef}
        className={`fixed top-0 right-0 h-full w-full bg-primary shadow-lg transition-all duration-300 max-w-2xl ${
          navbarOpen
            ? "translate-x-0 opacity-100"
            : "translate-x-full opacity-0"
        } z-50 px-6 sm:px-20 overflow-auto no-scrollbar`}
        role="dialog"
        aria-modal="true"
        ...
        <div className="flex flex-col h-full justify-between">
          <div>
            <div className="flex items-center justify-start py-10">
              <button
                ...
                className="bg-white p-3 rounded-full hover:cursor-pointer hover:bg-gray-100 transition-colors duration-300"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
          ...
          <div className="flex flex-col gap-3 my-16 text-white border-t border-white/20 pt-8">
            ...
            <div className="flex flex-col gap-2">
              <Link
                href="mailto:synertech42@yandex.ru"
                className="text-base sm:text-lg font-normal text-white/80 hover:text-white transition-colors duration-300 flex items-center gap-2"
                onClick={() => setNavbarOpen(false)}
              >
